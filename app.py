...
def chapter_totals(data):
    d={c:{"marks":0.0,"questions":0} for c in CHAPTERS}
    for q in data["questions"]:
        d[q["chapter"]]["marks"] += float(q["marks"])
        d[q["chapter"]]["questions"] += 1
    return d
...
def figure_crop(pdf_bytes,page_no,coords):
    ...
    try:
        ...
        page=doc[page_no-1]
        p=page.rect
        vals=[max(0,min(1,float(v))) for v in coords]
        candidate=fitz.Rect(p.x0+vals[0]*p.width,p.y0+vals[1]*p.height,
                            p.x0+vals[2]*p.width,p.y0+vals[3]*p.height)

        # Prefer actual PDF visual objects; never use text blocks.
        objects=[]
        try:
            for d in page.get_drawings():
                r=d.get("rect")
                if r and r.intersects(candidate) and r.width*r.height>8:
                    objects.append(r)
        except Exception:
            pass
        try:
        ...
        if objects:
            cx=(candidate.x0+candidate.x1)/2; cy=(candidate.y0+candidate.y1)/2
            inside=[r for r in objects if candidate.contains(fitz.Point((r.x0+r.x1)/2,(r.y0+r.y1)/2))]
            use=inside or objects
            union=use[0]
            for r in use[1:]: union |= r
            mx=min(10,p.width*.01); my=min(10,p.height*.01)
            crop=fitz.Rect(max(p.x0,union.x0-mx),max(p.y0,union.y0-my),
                           min(p.x1,union.x1+mx),min(p.y1,union.y1+my))
        else:
            crop=candidate
        ...
        # For scan fallback, only trim outer whitespace, not text selectively.
        if cv2 is not None and np is not None:
            arr=cv2.imdecode(np.frombuffer(png,np.uint8),cv2.IMREAD_GRAYSCALE)
            if arr is not None:
                ink=(arr<225).astype(np.uint8)*255
                pts=cv2.findNonZero(ink)
                if pts is not None:
                    x,y,w,h=cv2.boundingRect(pts)
                    pad=max(8,int(min(arr.shape)*.012))
                    x=max(0,x-pad); y=max(0,y-pad)
                    w=min(arr.shape[1]-x,w+2*pad); h=min(arr.shape[0]-y,h+2*pad)
                    ok,enc=cv2.imencode(".png",arr[y:y+h,x:x+w])
                    if ok: png=enc.tobytes()
        return png
    except Exception:
        return None
