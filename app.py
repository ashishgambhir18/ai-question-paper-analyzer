...
with st.sidebar:
    st.header("📚 Chapters")
    if st.session_state.get("data") and st.session_state.get("active_syllabus"):
        nav_groups=defaultdict(list)
        for qq in st.session_state["data"].get("questions",[]):
            nav_groups[qq.get("chapter","Unclassified")].append(qq)
        if st.button("🏠 Overview",use_container_width=True):
            st.session_state.pop("selected_chapter",None); st.rerun()
        for nn,cc in enumerate(CHAPTERS):
            if nav_groups.get(cc):
                if st.button(f"{cc}  ({len(nav_groups[cc])})",
                             key=f"nav_chapter_{nn}",use_container_width=True):
                    st.session_state["selected_chapter"]=cc; st.rerun()
    else:
        st.caption("Chapters will appear here after analysis.")
...
if st.button("🔍 Analyse Paper",type="primary",
    ...
    progress_bar=st.progress(0,text="0% completed")
    with st.status("Analysing paper...",expanded=True) as status:
        try:
            progress_label.markdown("### 5% completed")
            progress_bar.progress(5,text="5% completed")
            ...
            progress_bar.progress(25,text="25% completed")
            progress_label.markdown("### 25% completed")
            st.write(f"✓ {len(CHAPTERS)} syllabus units/chapters identified")

            st.write("📄 Reading question paper...")
            progress_bar.progress(35,text="35% completed")
            progress_label.markdown("### 35% completed")

            st.write("🔎 Extracting questions, marks and figures...")
            progress_bar.progress(45,text="45% completed")
            progress_label.markdown("### 45% completed")

            st.write("🧠 Classifying questions against the syllabus...")
            st.write("🎯 Analysing Bloom taxonomy, concepts and difficulty...")

            # The main AI analysis is one API request. We only advance the
            # percentage after that request actually completes, so the UI
            # never pretends that an arbitrary percentage is exact.
            st.session_state.data=analyse(paper_file)

            progress_bar.progress(80,text="80% completed")
            progress_label.markdown("### 80% completed")
            st.write(f"✓ {len(st.session_state.data.get('questions',[]))} questions processed")

            st.write("📐 Preparing detected figures for review...")
            progress_bar.progress(90,text="90% completed")
            progress_label.markdown("### 90% completed")

            st.write("📊 Preparing chapter weightage and Bloom summaries...")
            progress_bar.progress(98,text="98% completed")
            progress_label.markdown("### 98% completed")

            progress_bar.progress(100,text="100% completed")
            progress_label.markdown("### 100% completed ✅")
...
if not selected:
    ...
    st.info("Select a chapter from the left navigation to view its questions.")
