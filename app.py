...
def fetch_official_cisce_syllabus(board, subject, year=2027):
    ...
    The uploaded paper year is intentionally NOT used for syllabus selection.
    Historical papers are mapped to the 2027 syllabus so the analyzer shows
    how those questions relate to the current syllabus.
    """
    syllabus_year=2027
    ...
    headers={"User-Agent":"Mozilla/5.0 (compatible; AI-Question-Paper-Analyzer/1.1)"}
    resp=requests.get(page_url,headers=headers,timeout=30)
    resp.raise_for_status()
    soup=BeautifulSoup(resp.text,"html.parser")
    wanted=SUBJECT_ALIASES.get(subject,[subject])

    # First look for an exact subject heading.
    heading=None
    for tag in soup.find_all(["h1","h2","h3","h4","h5","strong","p","li"]):
        label=" ".join(tag.get_text(" ",strip=True).split())
        if any(label.lower()==w.lower() for w in wanted):
            heading=tag
            break

    # Some older CISCE pages put the subject inside a longer label such as
    # "Physics - Download". Keep the match constrained to the requested subject.
    if heading is None:
        for tag in soup.find_all(["h1","h2","h3","h4","h5","strong","p","li"]):
            label=" ".join(tag.get_text(" ",strip=True).split())
            low=label.lower()
            if any(w.lower() in low for w in wanted) and len(label) <= 120:
                heading=tag
                break

    if heading is None:
        raise ValueError(
            f"Could not find the '{subject}' syllabus on the official CISCE {board} 2027 page: {page_url}"
        )

    # Prefer a PDF link belonging to the same subject block/card.
    href=None
    containers=[]
    cur=heading
    for _ in range(4):
    ...
    for container in containers:
        for a in container.find_all("a",href=True):
            ...
            if ".pdf" in h.lower() and (not txt or any(w.lower() in (txt+" "+h).lower() for w in wanted)):
                href=h
                break
        if href:
            break

    # Fallback to nearby links, but stop at the next subject heading.
    if not href:
        for a in heading.find_all_next("a",href=True,limit=60):
            h=a.get("href","")
            if ".pdf" in h.lower():
                href=h
                break
    ...
    if not href:
        raise ValueError(
            f"The official CISCE {board} 2027 page was found, but no PDF download link was found for {subject}."
        )
    ...
    if not pdf_resp.content.startswith(b"%PDF"):
        raise ValueError(f"CISCE returned a non-PDF response for {subject} (2027).")
