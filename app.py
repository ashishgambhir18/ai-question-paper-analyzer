    ...
    for c in CHAPTERS:
        ...
        for q in groups[c]:
            ...
            if pdf_bytes and q.get("has_figure"):
                ...
                img=manual_crops.get(crop_key)
                if img is not None:
                    outbuf=io.BytesIO()
                    img.save(outbuf,format="PNG")
                    img_bytes=outbuf.getvalue()
                else:
...
with st.sidebar:
    ...
    # A bundled syllabus can be added later for any combination.
    syllabus=st.session_state.get("active_syllabus")
    if syllabus and (syllabus.get("board"),str(syllabus.get("year")),syllabus.get("class"),syllabus.get("subject"))==(board,str(year),klass,subject):
        CHAPTERS=[u["name"] for u in syllabus["units"]]
        st.success("Official syllabus loaded")
        st.caption(syllabus["source_url"])
    else:
        CHAPTERS=list(DEFAULT_CHAPTERS)
        st.info("Load the official syllabus for this subject/year.")
        if st.button("🌐 Find Official CISCE Syllabus",use_container_width=True):
            links,err=official_links(board,year,subject)
            if err: st.error(err)
            elif links: st.session_state["official_links"]=links
            else: st.warning("No matching official document found automatically.")

        links=st.session_state.get("official_links",[])
        if links:
            pick=st.selectbox("Official document",range(len(links)),
                              format_func=lambda i:links[i]["title"][:110],key="official_pick")
            if st.button("Use selected official document",use_container_width=True):
                try:
                    u=links[pick]["url"]
                    r=requests.get(u,headers={"User-Agent":"Mozilla/5.0"},timeout=60); r.raise_for_status()
                    st.session_state["syllabus_raw"]=r.content
                    st.session_state["syllabus_name"]=u.split("/")[-1] or "official.pdf"
                    st.session_state["syllabus_source"]=u
                except Exception as e: st.error(f"Download failed: {e}")

        su=st.file_uploader("Or upload official CISCE syllabus PDF",type=["pdf"],key="syllabus_upload")
        if su:
            st.session_state["syllabus_raw"]=su.getvalue()
            st.session_state["syllabus_name"]=su.name
            st.session_state["syllabus_source"]="Official CISCE syllabus uploaded by user"
...
if st.button("🔍 Analyse Paper",type="primary",disabled=upload is None,use_container_width=True):
    ...
    with st.spinner("Loading syllabus and analysing the paper..."):
        try:
            syllabus=st.session_state.get("active_syllabus")
            match=(syllabus and (syllabus.get("board"),str(syllabus.get("year")),
                                  syllabus.get("class"),syllabus.get("subject"))==(board,str(year),klass,subject))
            if not match:
                raw=st.session_state.get("syllabus_raw")
                if not raw:
                    st.error("Please select/download or upload the official CISCE syllabus first.")
                    st.stop()
                syllabus=extract_syllabus_units(
                    raw,st.session_state.get("syllabus_name","official.pdf"),
                    board,year,klass,subject,
                    st.session_state.get("syllabus_source","Official CISCE syllabus")
                )
                st.session_state["active_syllabus"]=syllabus
            CHAPTERS=[u["name"] for u in syllabus["units"]]
            st.session_state.data=analyse(upload)
...
if st.session_state.get("active_syllabus"):
    st.caption(f"Official syllabus: {st.session_state['active_syllabus']['source_url']}")
...
if abs(extracted_total-printed) > 0.01:
    st.warning(
        ...
        f"{extracted_total:g} marks, which differs from the "
        f"{printed:g}-mark printed paper by {extracted_total-printed:+g}. "
    ...
    for i,q in groups[c]:
        with st.container(border=True):
            ...
            if key in st.session_state.answers:
                with st.expander("💡 AI Answer",expanded=True): st.markdown(render_ai_answer(st.session_state.answers[key]))
