            ...
            "questions":{
                "type":"array","items":{
                    ...
                    "required":[
                        ...
                        "chapter","marks","concept","question_type","bloom_level",
                        "bloom_reason","difficulty",
...
- Identify concept, question type and difficulty.
- Classify every question using revised Bloom's Taxonomy:
  Remember = recall facts, definitions, laws or formulas.
  Understand = explain, describe, interpret or summarize.
  Apply = use a known principle, formula or procedure in a situation.
  Analyze = break information into parts, compare, infer relationships or causes.
  Evaluate = justify, critique, assess, or defend a conclusion with evidence.
  Create = design, formulate, construct or propose something new.
- Choose the highest cognitive process genuinely required to answer the question.
  Do not classify every numerical as Apply automatically.
- Give a concise bloom_reason.
...
def bloom_totals(data):
    ...
    totals={x:{"marks":0.0,"questions":0} for x in levels}
...
def make_word(data,pdf_bytes=None,with_answers=False):
    ...
    bt=bloom_totals(data); btotal=sum(v["marks"] for v in bt.values())
    ...
    doc.add_paragraph("Classification uses the highest cognitive process genuinely required by each question.")
    ...
    for c in CHAPTERS:
        ...
        for q in groups[c]:
            ...
            meta.add_run(" | Bloom: ").bold=True; meta.add_run(q.get("bloom_level",""))
...
brows=[{"Bloom Level":lvl,"Marks":bt[lvl]["marks"],"Questions":bt[lvl]["questions"]}
...
if brows:
    ...
    bx,by=st.columns(2)
    with bx:
        st.plotly_chart(px.pie(bdf,names="Bloom Level",values="Marks",hole=.4,
                                title="Marks by Cognitive Level"),
                        use_container_width=True)
    with by:
        st.plotly_chart(px.bar(bdf,x="Bloom Level",y="Marks",
                               title="Marks by Bloom Level"),
                        use_container_width=True)
    btotal=bdf["Marks"].sum()
    bdf2=bdf.copy()
    bdf2["Weightage %"]=(bdf2["Marks"]/btotal*100).round(1).astype(str)+"%"
    st.dataframe(bdf2,use_container_width=True,hide_index=True)
    with st.expander("ℹ️ How Bloom classification works"):
        st.write("Each question is classified by the highest cognitive process genuinely required to answer it. A numerical question is not automatically classified as Apply.")
