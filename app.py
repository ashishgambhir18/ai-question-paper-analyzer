...
For ISC, return only the selected class."""
    ...
    return {"board":board,"year":year,"class":klass,"subject":subject,
...
/* Keep each question visually isolated and prevent Streamlit blocks from
   appearing to overlap when figures, expanders or buttons have different heights. */
[data-testid="stVerticalBlockBorderWrapper"]{
  margin-bottom:1.35rem !important;
  padding-bottom:.75rem !important;
  overflow:visible !important;
}
.question-meta{display:flex;flex-wrap:wrap;gap:.45rem;margin:.7rem 0 1rem}
.meta-badge{display:inline-block;padding:.28rem .62rem;border-radius:999px;
font-size:.82rem;font-weight:650;line-height:1.25;border:1px solid transparent}
.meta-bloom{background:#f3e8ff;color:#7a27b0;border-color:#dfc5ff}
.meta-marks{background:#e9f8ef;color:#177245;border-color:#bfe7ce}
.meta-type{background:#e8f1ff;color:#1858a8;border-color:#bfd5fa}
.meta-difficulty{background:#fff2df;color:#a45a00;border-color:#f5d2a1}
.meta-concept{background:#e8f8f7;color:#14746d;border-color:#bde5e1}
.question-text{font-size:1.04rem;line-height:1.65;margin-bottom:.2rem}
</style>
...
if st.button("🔍 Analyse Paper",type="primary",
    ...
    # Use real Streamlit progress tied to completed analysis stages/questions.
    progress_title = st.empty()
    progress_bar = st.progress(0, text="Starting analysis…")
    progress_title.markdown("### 0% completed")

    def set_overall_progress(pct, message):
        pct = max(0, min(100, int(pct)))
        progress_title.markdown(f"### {pct}% completed")
        progress_bar.progress(pct, text=message)

    with st.status("Analysing paper...",expanded=True) as status:
        try:
            st.write("📚 Reading the uploaded syllabus...")
            set_overall_progress(5, "Reading uploaded syllabus…")
            ...
            set_overall_progress(15, "Syllabus ready — reading question paper…")
            ...
            def update_question_progress(done,total):
                # Classification accounts for the middle 80% of overall progress.
                local_pct = 100 if not total else int(done*100/total)
                overall_pct = 15 + int(local_pct * 0.80)
                set_overall_progress(overall_pct, f"Sorting questions: {done}/{total} ({local_pct}%)")
                question_progress.progress(local_pct, text=f"Sorting questions: {done}/{total} ({local_pct}%)")

            st.write("🧠 Classifying each question against the uploaded syllabus...")
            ...
            # Finish the real progress widgets at 100%.
            set_overall_progress(100, "Analysis complete")
            progress_title.markdown("### 100% completed ✅")
            ...
            progress_title.markdown("### Analysis stopped ❌")
...
if not selected:
    ...
    st.header("📝 Sorted Question Paper")
    st.caption("The questions below are the original extracted question text. Classification metadata is shown separately; AI answers are never inserted into the question.")
    overview_groups=defaultdict(list)
    for qq in data.get("questions", []):
        overview_groups[qq.get("chapter", "Unclassified")].append(qq)

    for cc in CHAPTERS:
        if not overview_groups.get(cc):
            continue
        st.markdown(f"### 📚 {cc}")
        for qq in overview_groups[cc]:
            with st.container(border=True):
                st.markdown(f"**Q{qq.get('question_no','')}.** {qq.get('question_text','')}")
                st.caption(
                    f"Marks: {qq.get('marks',0):g} • {qq.get('question_type','')} • "
                    f"Bloom: {qq.get('bloom_level','Not classified')} • Concept: {qq.get('concept','')}"
                )
    st.header("🎯 Main Concepts to Focus On")
    for x in data.get("focus_concepts", []):
...
for i,q in selected_questions:
    with st.container(border=True):
        st.markdown(
            f'<div class="question-text"><strong>Q{q["question_no"]}.</strong> {q["question_text"]}</div>',
            unsafe_allow_html=True
        )
        st.markdown(
            f'<span class="tag">Chapter: {q["chapter"]}</span>'
            f'<div class="question-meta">'
            f'<span class="meta-badge meta-bloom">🟣 Bloom: {q.get("bloom_level","Not classified")}</span>'
            f'<span class="meta-badge meta-marks">🟢 {q["marks"]} Mark' + ('s' if float(q.get("marks",0) or 0) != 1 else '') + '</span>'
            f'<span class="meta-badge meta-type">🔵 {q.get("question_type","Question")}</span>'
            f'<span class="meta-badge meta-difficulty">🟠 {q.get("difficulty","Not classified")}</span>'
            f'<span class="meta-badge meta-concept">📘 {q.get("concept","")}</span>'
            f'</div>',
            unsafe_allow_html=True
        )
