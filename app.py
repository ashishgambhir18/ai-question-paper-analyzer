...
def fetch_official_cisce_syllabus(board, subject):
    page_url=CISCE_2027_PAGES[board]
    headers={"User-Agent":"Mozilla/5.0 (compatible; AI-Question-Paper-Analyzer/1.0)"}
    ...
    for tag in soup.find_all(["h2","h3","h4","strong","p"]):
        ...
        if any(label.lower()==w.lower() for w in wanted):
            ...
            break
    if heading is None:
        raise ValueError(f"Could not find '{subject}' on the official CISCE {board} 2027 syllabus page.")

    href=None
    for a in heading.find_all_next("a",href=True,limit=40):
        h=a.get("href","")
        if ".pdf" in h.lower():
            href=h
            break
    if not href:
        raise ValueError(f"CISCE page was found, but no PDF download link was found for {subject}.")
    ...
    if not pdf_resp.content.startswith(b"%PDF"):
        raise ValueError(f"CISCE returned a non-PDF response for {subject}.")
...
st.caption("Upload the question paper. The app automatically retrieves the applicable syllabus from the official CISCE website and uses it as the source of truth for chapter/unit classification.")

c1,c2,c3,c4=st.columns(4)
with c1: board=st.selectbox("Board",["ICSE","ISC"])
with c2: year=st.selectbox("Examination Year",list(range(2027,2014,-1)),index=0)
...
if st.button("🔍 Analyse Paper",type="primary",
    ...
    with st.status("Analysing paper...",expanded=True) as status:
        try:
            st.write("🌐 Connecting to the official CISCE syllabus...")
            if int(year) != 2027:
                raise ValueError(
                    "Automatic CISCE syllabus retrieval is currently configured for 2027. "
                    "Older examination years can be added using their official CISCE pages."
                )
            syllabus_bytes,syllabus_name,syllabus_url=fetch_official_cisce_syllabus(board,subject)
