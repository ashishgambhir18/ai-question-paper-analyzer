...
def show_manual_crop(pdf_bytes,page_no,crop_key):
    ...
    st.caption("Drag the box around the complete figure. Include labels/arrows/axes/wires; exclude question text.")
    return st_cropper(image,realtime_update=True,box_color="#ff4b4b",
                      aspect_ratio=None,return_type="image",
                      key=f"manual_crop_{crop_key}")
...
with st.sidebar:
    ...
    st.info("Testing mode: upload BOTH the syllabus PDF and the question paper PDF.")
    ...
    syllabus_file=st.file_uploader("1️⃣ Upload Syllabus PDF",type=["pdf"],key="syllabus_upload")
    paper_file=st.file_uploader("2️⃣ Upload Question Paper PDF",type=["pdf"],key="paper_upload")
    st.caption("Later, syllabus PDFs can be stored in a common folder so users only upload the question paper.")


paper_file=st.session_state.get("paper_upload")
if st.button("🔍 Analyse Paper",type="primary",
    ...
    with st.spinner("Reading syllabus and analysing the paper..."):
        try:
...
for c in CHAPTERS:
    qs=groups.get(c,[])
    if qs:
        marks=sum(float(q.get("marks",0) or 0) for _,q in qs)
        chapter_rows.append((c,len(qs),marks,round(marks/printed*100,1) if printed else 0))

st.caption("Select a chapter/unit. Questions from other chapters are hidden.")

for n,(chapter,nq,marks,wgt) in enumerate(chapter_rows):
    a,b,c,d=st.columns([5,1,1,1])
    with a:
        if st.button(f"📂 {chapter}",key=f"open_chapter_{n}",use_container_width=True):
            st.session_state["selected_chapter"]=chapter
            st.rerun()
    b.metric("Questions",nq)
    c.metric("Marks",f"{marks:g}")
    d.metric("Weightage",f"{wgt}%")
...
st.divider()
if st.button("← Back to Chapters / Units",key="back_to_chapters"):
    st.session_state.pop("selected_chapter",None)
    st.rerun()

selected_questions=groups[selected]
...
for i,q in selected_questions:
    with st.container(border=True):
        ...
        st.markdown(f"**Bloom: {q.get('bloom_level','Not classified')}**  • Marks: {q['marks']} • {q['concept']} • {q['question_type']} • {q['difficulty']}")
        ...
        if q.get("has_figure"):
            ...
            if saved is not None:
                st.image(saved,caption="Selected original figure",use_container_width=True)
                if st.button("✂️ Re-crop Figure",key=f"recrop_{i}",use_container_width=True):
                    st.session_state[f"crop_mode_{crop_key}"]=True
                    st.rerun()
            else:
                if st.button("✂️ Crop Figure Manually",key=f"crop_{i}",use_container_width=True):
                    st.session_state[f"crop_mode_{crop_key}"]=True
                    st.rerun()
            if st.session_state.get(f"crop_mode_{crop_key}",False):
                cropped=show_manual_crop(st.session_state.source_bytes,q.get("source_page",1),crop_key)
                if cropped is not None:
                    st.image(cropped,caption="Crop preview",use_container_width=True)
                    x,y=st.columns(2)
                    with x:
        ...
        with st.expander("👩‍🏫 Teacher Review"):
            choice=st.selectbox("Chapter / Unit",CHAPTERS,index=CHAPTERS.index(q["chapter"]),key=f"ch_{i}")
            if choice!=q["chapter"]:
